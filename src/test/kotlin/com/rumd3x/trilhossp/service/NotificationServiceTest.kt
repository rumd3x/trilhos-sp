...
class NotificationServiceTest {
    ...
    @Test fun `notify returns empty when line is not in allowed list`() {
        StepVerifier
            .create(service(lines = "5,9").notify(line, diff("Operação Normal", "Lentidão")))
            .verifyComplete()
    }
    ...
    @Test fun `notify calls send when line is in allowed list`() {
        ...
        service(lines = "4,5", notifiers = listOf(notifier { called.set(true) }))
            .notify(line, diff("Operação Normal", "Lentidão"))
            .block()
    ...
    @Test fun `notify returns empty when today is not in allowed days`() {
        ...
        StepVerifier
            .create(service(days = otherDays).notify(line, diff("Operação Normal", "Lentidão")))
            .verifyComplete()
    }
    ...
    @Test fun `notify returns empty when diff level is below threshold`() {
        ...
        StepVerifier
            .create(service(level = 3).notify(line, diff("Lentidão", "Operação Normal")))
            .verifyComplete()
    }
